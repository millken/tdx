...
func DecodeFinance(body []byte) (*FinanceInfo, error) {
	...
	off := 0
	info.LiuTongGuBen = float64(readFloat32(bs[off:]))
	off += 4
	info.Province = binary.LittleEndian.Uint16(bs[off:])
	off += 2
	info.Industry = binary.LittleEndian.Uint16(bs[off:])
	off += 2
	info.UpdatedDate = binary.LittleEndian.Uint32(bs[off:])
	off += 4
	info.IPODate = binary.LittleEndian.Uint32(bs[off:])
	off += 4

	info.ZongGuBen = float64(readFloat32(bs[off:]))
	off += 4
	info.GuoJiaGu = float64(readFloat32(bs[off:]))
	off += 4
	info.FaQiRenFaRenGu = float64(readFloat32(bs[off:]))
	off += 4
	info.FaRenGu = float64(readFloat32(bs[off:]))
	off += 4
	info.BGu = float64(readFloat32(bs[off:]))
	off += 4
	info.HGu = float64(readFloat32(bs[off:]))
	off += 4

	info.MeiGuShouYi = float64(readFloat32(bs[off:]))
	off += 4
	info.ZiChanZongJi = float64(readFloat32(bs[off:]))
	off += 4
	info.LiuDongZiChanZongJi = float64(readFloat32(bs[off:]))
	off += 4
	info.GuDingZiChanJinE = float64(readFloat32(bs[off:]))
	off += 4
	info.WuXingZiChan = float64(readFloat32(bs[off:]))
	off += 4
	info.GuDongRenShu = float64(readFloat32(bs[off:]))
	off += 4

	info.LiuDongFuZhaiHeJi = float64(readFloat32(bs[off:]))
	off += 4
	info.ChangQiFuZhai = float64(readFloat32(bs[off:]))
	off += 4
	info.ZiBenGongJiJin = float64(readFloat32(bs[off:]))
	off += 4
	info.GuiMoQuanYiHeJi = float64(readFloat32(bs[off:]))
	off += 4

	info.YinYeZongShouRu = float64(readFloat32(bs[off:]))
	off += 4
	info.YinYeChengBen = float64(readFloat32(bs[off:]))
	off += 4
	info.YingShouZhangKuan = float64(readFloat32(bs[off:]))
	off += 4
	info.YinYeLiRun = float64(readFloat32(bs[off:]))
	off += 4
	info.TouZiShouYi = float64(readFloat32(bs[off:]))
	off += 4

	info.JingYingXianJinLiu = float64(readFloat32(bs[off:]))
	off += 4
	info.ZongXianJinLiu = float64(readFloat32(bs[off:]))
	off += 4
	info.CunHuo = float64(readFloat32(bs[off:]))
	off += 4
	info.LiRunZongE = float64(readFloat32(bs[off:]))
	off += 4
	info.ShuiHouLiRun = float64(readFloat32(bs[off:]))
	off += 4

	info.GuiMoJinLiRun = float64(readFloat32(bs[off:]))
	off += 4
	info.WeiFenLiRun = float64(readFloat32(bs[off:]))
	off += 4
	info.MeiGuJingZiChan = float64(readFloat32(bs[off:]))
	off += 4
	info.BaoLiu2 = float64(readFloat32(bs[off:]))
	off += 4
